...
if __name__ == "__main__":
    ...
    async def test_cycle_validator():
        ...
        try:
            ...
            print(f"🔑 API 인증 상태: {'인증됨' if validator.trader.okx_client.auth_available else '공개 API만 사용'}")
            ...
            if validator.trader.okx_client.auth_available:
                print("\n💡 실제 4구간 검증을 실행하려면:")
                print("   await validator.run_complete_4tier_validation()")
            else:
                print("\n💡 실제 검증을 위해서는 .env 파일에 OKX API 키를 설정하세요")
