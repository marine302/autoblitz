...
class OKXCoinInfoCollector:
    ...
    async def collect_all_spot_instruments(self):
        ...
        for instrument in instruments:
            ...
            if min_sz == 0 or lot_sz == 0 or tick_sz == 0:
                print(
                    f"⚠️ 건너뜀: {symbol} (min:{min_sz_str}, lot:{lot_sz_str}, tick:{tick_sz_str})")
                skipped_count += 1
                continue
        ...
        if symbols:
            ...
            if tickers:
                for ticker in tickers:
                    last_str = ticker.get('last', 0)
                    try:
                        last_price = float(last_str) if last_str not in (
                            '', None) else 0.0
                    except ValueError:
                        last_price = 0.0
                    price_info = {
        ...
        for symbol, spec in coin_specs.items():
            ...
            if symbol in prices:
                ...
                if price_info['last_price'] > 0:
                    min_order_usdt = spec['trading_rules']['min_order_size'] * \
    ...
    async def update_specific_coins(self, symbols):
        ...
        for symbol in symbols:
            if symbol in existing_data['coins'] and symbol in updated_prices:
                ...
                existing_data['coins'][symbol]['metadata']['last_updated'] = datetime.now(
                ).isoformat()
                updated_count += 1
        ...
        if updated_count > 0:
            self.save_coin_data(
                existing_data['coins'], "okx_coins_updated.json")
    ...
    async def collect_all_data(self):
        ...
        # 3. 정보 결합
        enhanced_specs = self.enhance_coin_specs_with_prices(
            coin_specs, prices)
